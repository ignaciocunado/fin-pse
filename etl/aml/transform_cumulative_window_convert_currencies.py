...
def _prep(df: pl.DataFrame) -> pl.LazyFrame:
    ...
    boundaries = (
        pl.datetime_range(start=start, end=end, interval=every, eager=True)
        .to_frame("window_start")  # Should be window end, but we keep it window_start to avoid changing code
        .lazy()
    )

    return boundaries.join(lf, how="cross").filter(pl.col("Timestamp") <= pl.col("window_start"))
...
def flow_targets_out_entropy_amount_fast(trans: DataFrame, k2: bool = True) -> DataFrame:
    ...
    W = lf.group_by(["window_start", "From", "To"]).agg(pl.col("Amount").sum().alias("w"))

    P = (
        W.with_columns(pl.col("w").sum().over(["window_start", "From"]).alias("out_wsum"))
